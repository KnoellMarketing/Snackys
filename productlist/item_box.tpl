...
<div id="result-wrapper_buy_form_{$Artikel->kArtikel}" class="p-c {if $snackyConfig.hover_productlist === 'Y' && !$device->isMobile()} hv-e{/if}{if isset($listStyle) && $listStyle === 'gallery'} active{/if}{if isset($class)} {$class}{/if}">
    ...
    <a class="img-w block" href="{$Artikel->cURL}">
        ...
        {if $viewportImages < 4} {include file="snippets/image.tpl" src=$Artikel->Bilder[0]->cPfadKlein alt=$alt second=$Artikel->Bilder[1]->cPfadKlein noLazy=1}
            ...
            {/if}
            {else}
            {if $viewportImages < 4} {include file="snippets/image.tpl" src=$Artikel->Bilder[0]->cPfadKlein alt=$alt noLazy=1}
                {assign var="viewportImages" value=$viewportImages+1 scope="global"}
                {else}
    ...
    <div class="caption">
        <a href="{$Artikel->cURL}" class="title block h4 m0" itemprop="name">{$Artikel->cKurzbezeichnung}</a>
        {if $Artikel->cName !== $Artikel->cKurzbezeichnung}
        <meta itemprop="alternateName" content="{$Artikel->cName}">{/if}
    ...
    <form id="buy_form_{$Artikel->kArtikel}" action="navi.php" method="post" class="form form-basket evo-validate" data-toggle="basket-add">
        ...
        <div class="delivery-status">
            ...
            $Artikel->fLagerbestand <= 0 && $Artikel->fZulauf > 0 && isset($Artikel->dZulaufDatum_de)}
                ...
                <div class="signal_image status-1 small">{lang key="productInflowing" section="productDetails" printf=$cZulauf}</div>
                {elseif $anzeige !== 'nichts' &&
                $Einstellungen.artikeluebersicht.artikeluebersicht_lagerbestandanzeige_anzeigen !== 'N' &&
                $Artikel->cLagerBeachten === 'Y' && $Artikel->fLagerbestand <= 0 && $Artikel->fLieferantenlagerbestand > 0 && $Artikel->fLieferzeit > 0 &&
                    ($Artikel->cLagerKleinerNull === 'N' ||
                    $Einstellungen.artikeluebersicht.artikeluebersicht_lagerbestandanzeige_anzeigen === 'U')}
                    <div class="signal_image status-1 small">{lang key="supplierStockNotice" section="global" printf=$Artikel->fLieferzeit}</div>
                    ...
                    <div class="estimated_delivery hidden-xs small">{lang key="shippingTime" section="global"}: {$Artikel->cEstimatedDelivery}</div>
                    {/if}
        </div>
        {/block}
                ...
                <div class="quantity-wrapper form-group top7">
                    ...
                    <div class="quantity-wrapper form-group top7">
                        <div class="input-group input-group-sm">
                            <input type="number" min="0" {if $Artikel->fAbnahmeintervall > 0}step="{$Artikel->fAbnahmeintervall}"{/if}
                            ...
                            <span class="input-group-btn">
                                <button type="submit" class="btn btn-primary" id="submit{$Artikel->kArtikel}" title="{lang key=" addToCart" section="global" }">
        ...
        <div class="exp{if $snackyConfig.listShowCart == 2} flo-bt {/if}{if $snackyConfig.listShowAmountCart == 1} hide-qty{/if}">
            {block name="form-expandable"}
            {block name="productlist-add-basket"}
            {if ($Artikel->inWarenkorbLegbar === 1 || ($Artikel->nErscheinendesProdukt === 1 && $Einstellungen.global.global_erscheinende_kaeuflich === 'Y')) &&
            ...
            {if $Artikel->nIstVater && $Artikel->kVaterArtikel == 0}
            {else}
            {if $snackyConfig.listShowCart == 3 && $snackyConfig.listShowAmountCart == 2}<div class="input-group input-group-sm small">{/if}
                <input type="number" min="0" {if $Artikel->fAbnahmeintervall > 0}step="{$Artikel->fAbnahmeintervall}"{/if}
                id="quantity{$Artikel->kArtikel}" class="quantity form-control text-right" name="anzahl"
                autocomplete="off"
                ...
                <span class="input-group-btn">
                    <button type="submit" class="btn btn-primary sn-addBasket" id="submit{$Artikel->kArtikel}" title="{lang key=" addToCart" section="global" }">
                ...
                {if $snackyConfig.listShowCart == 3 && $snackyConfig.listShowAmountCart == 2}</div>{/if}
            {/if}
            {elseif $snackyConfig.listShowCart == 3}
            <a href="{$Artikel->cURL}" class="btn btn-block">{lang key="details"}</a>
            {/if}
            {/block}
