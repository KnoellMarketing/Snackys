...
{else}
    ...
    {block name="content"}
        ...
        {if !empty($Link->Sprache->cContent) && $Link->nLinkart != $smarty.const.LINKTYP_STARTSEITE}
            {if $Einstellungen.template.general.optimize_content == "Y"}{$Link->Sprache->cContent|optimize}{else}{$Link->Sprache->cContent}{/if}
        {/if}
        ...
        {elseif $Link->nLinkart == $smarty.const.LINKTYP_STARTSEITE}
            ...
            {if !empty($Link->Sprache->cContent)}
                <div class="custom-content">{if $Einstellungen.template.general.optimize_content == "Y"}{$Link->Sprache->cContent|optimize}{else}{$Link->Sprache->cContent}{/if}</div>
            {/if}
